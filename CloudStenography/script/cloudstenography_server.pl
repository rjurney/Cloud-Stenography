...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{CLOUDSTENOGRAPHY_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{CLOUDSTENOGRAPHY_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
my $runner = sub {
    ...
    require CloudStenography;

    CloudStenography->run(
        $port, $host,
        {
            argv       => \@argv,
            'fork'     => $fork,
...
cloudstenography_server.pl - Catalyst Testserver

=head1 SYNOPSIS

cloudstenography_server.pl [options]
